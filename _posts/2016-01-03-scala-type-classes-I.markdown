...
В Scala, классы типов организованы на базе типов высшего порядка и implcit'ов. Если в параметрическом полиморфизме, в классе `C[T]` мы определяем метод, он работает одинаково для любых параметров - реализация знает только о некотором типе `T`. В случае же "ad-hoc" полиморфизма, функция имеет разные реализации для разных типов аргументов, например, для `1+2` или `“x”+”y”` у нас нужно будет предоставить 2 разных реализации. 
