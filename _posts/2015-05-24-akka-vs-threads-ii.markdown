...
В Akka есть поддержка этой парадигмы. Akka STM фактически превращает динамическую память JVM в транзакционный источник данных с семантикой `begin/commit/rollback` - очень похоже на обычную транзакционную БД, только без хранилища. Можно сказать, что она поддерживает три первые буквы в слове *ACID*; 
