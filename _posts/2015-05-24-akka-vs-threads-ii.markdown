...
Если же нам нужно создать пул акторов, которые бы разгребали очередь сообщений и работали наподобие `ForkJoin` пула, то мы можем создать следующий диспетчер (*dispatcher*):
...
Диспетчер с перехватом работы (Work Stealing Dispatcher) который базируется на пуле потоков фиксированного размера, и осуществляет балансировку в тех акторов, для которых он используется с учетом их загрузки.
...
3. Акторы помогают улучшить надежность системы. Например, мы можем настроить стратегию супервизора таким образом, что упавший актор будет перезапущен, и решение об этом может принимать актор-родитель, а необязательно непосредственно сам актор. Например, пусть у нас есть 3 актора в иерархии, родитель, дочерний актор и дочерний актор дочернего актора. Родитель переопределяет `supervisorStrategy`: 
...
* Номер порта, который будет слушать данная система акторов
...
Также, при использовании  *Message passing concurrency* могут возникать ситуации, которые не решаются на 100% только лишь передачей сообщений.
...
* Согласованность (*consistency*): транзакция должна оставить данные в согласованном состоянии. В Akka уровень изоляции соотвествует уровню *SERIALIZED* в СУБД Oracle. 

* Изолированность: изменения, сделанные в других транзакциях, не должны быть видны в данной транзакции.
